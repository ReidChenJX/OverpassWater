...
rain_flood = pd.read_csv('../data/data/hydrops_data.csv', encoding='gbk')
overpass_abute = pd.read_csv('../data/data/overpass_abute.csv', encoding='gbk')
rain_data = pd.read_csv('../data/data/rain_data.csv', encoding='gbk')
...
rain_flood[['S_STATIONID','START_RAIN','END_RAIN','DURATION','N_RAINVALUE','RARANK', 'DEFINE']] = ''
...
for i in range(rain_flood_len):
    ...
    for one_rain in one_allRain.itertuples(index=True):
        ...
        if one_rain.START_TIME <=rain_flood.loc[i,'START_TIME'] <= one_rain.END_TIME :
            ...
            rain_flood.loc[i, 'RARANK'] = one_rain.RARANK
...
rain_flood.to_csv('../data/data/rain_flood.csv',index=False,encoding='gbk')
