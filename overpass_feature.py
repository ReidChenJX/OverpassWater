...
def anomalyData(overpass8_9data):
    ...
    for row in overpass8_9data.itertuples(index=True):
        ...
        # 若为异常情况，需要记录到overpass_neg内
            if row.S_NO not in S_NO:
                ...
                # 若离线则设置为1
                if row.S_STATENAME != '正常': LINE_Off.append(row.S_STATENAME)
                else: LINE_Off.append('正常')
                # 若无地址，设置为1
                if row.S_ADDR == '无': NO_ADDR.append('无')
                else: NO_ADDR.append(row.S_ADDR)
                # 若无降雨站，设置为1
                if row.S_STATIONID == np.nan: NO_RAIN.append('无')
                else: NO_RAIN.append(row.S_STATIONID)
        # 若为正常情况，继续循环
        else:
            continue
    # 将数据整合至overpass_neg
    ...
    # 将数据写入csv文件
    neg_path = './data/overpass_neg.csv'
...
def hydrops_data(pos_index):
        ...
        for value in JSD_value.itertuples(index=False):
            ...
            if value.N_VALUE < 10 and log == 1:  # 正在积水但在此时退出积水
                ...
                if 10 <= water_deep < 15:  # 判断积水深度等级
                    ...
                    rank = 4
                hydrops_data.loc[hydrops_data_index] = [value.S_NO, start_time, end_time, water_deep, rank]
    ...
    hydrops_data.to_csv('./data/hydrops_data.csv', encoding='gbk', index=False)
    return hydrops_data
